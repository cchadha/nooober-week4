async function pageLoaded() {
  ...
  for (let i=0; i<json.length; i++) {
    ...
    <h1 class = "inline-block mt-8 px-4 py-2 rounded-xl text-2xl big-clip-text text-transparent bg-gradient-to-r from-blue-500 to-purple-500">
    ...
    <div class = "flex">
    <div class = "w-1/2">
    <h2 class = "text-2xl py-1">${ride[0].passengerDetails.first}
    ${ride[0].passengerDetaials.last}</h2>
    ...
    <p>${ride[0].pickupLocation.city}, ${ride[0].pickupLocation.state} 
    ${ride[0].pickupLocation.zip}</p>
    </div>
    <div class="w-1/2">
    <div class="text-sm font-bold text-gray-600">DROPOFF</div>
    <p>${ride[0].dropoffLocation.address}</p>
    <p>${ride[0].dropoffLocation.city}, ${ride[0].dropoffLocation.state} 
    ${ride[0].dropoffLocation.zip}</p>
    </div>
    </div>
    <div> `) 
  }
